...
class HolidayDateTime extends DateTime {
    ...
    public function checkHoliday() {
        ...
        foreach (self::$holidays as $holiday) {
            list($method, $month, $date, $start, $end, $name) = $holiday;
            //$method .= 'Holiday';
            //$result = $this->$method($month, $day, $start, $end, $name);
            switch ($method) {
                case 'fixed':
                    $result = $this->fixedHoliday($month, $date, $start, $end, $name);
                    if ($result == '元旦') {
                        $a=0;
                    }
                    break;
                case 'happy':
                    $result = $this->happyHoliday($month, $date, $start, $end, $name);
                    if ($result == '元旦') {
                        $a=0;
                    }
                    break;
                case 'spring':
                    $result = $this->springHoliday($month, $date, $start, $end, $name);
                    if ($result == '元旦') {
                        $a=0;
                    }
                    break;
                case 'autumn':
                    $result = $this->autumnHoliday($month, $date, $start, $end, $name);
                    if ($result == '元旦') {
                        $a=0;
                    }
                    break;
     ...
     * 固定祝日かどうか
     */
    private function fixedHoliday($month, $date, $start, $end, $name) {
        $Month = intval(date('m', strtotime($this->date)));
        $Date = intval(date('d', strtotime($this->date)));
        if ($Month != $month) {
            return false;
        }
        if ($Date != $date) {
    ...
    private function happyHoliday($month, $week, $start, $end, $name) {
        $Year = intval(date('Y', strtotime($this->date)));
        $Month = intval(date('m', strtotime($this->date)));
        $Day = intval(date('w', strtotime($this->date)));
        if ($Month != $month)
            return false;
        // 第*月曜日の日付を求める
        $w = 1; // 月曜日固定
